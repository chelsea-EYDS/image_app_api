...
export class ClarifaiService {
  ...
  public async addInputs(addInputsDto: AddInputsDto) {
    ...
    try {
      const response = await clar.post('/v2/inputs', JSON.stringify(data))
      return response.data
    } catch (err) {
      return err.response
    }
  ...
  public async trainModel() {
    try {
      const response = await clar.post(`/v2/users/me/apps/${process.env.USER_APP_ID}/models/custom_model/versions`)
      return response?.data
    } catch (err) {
      return err
    }
  ...
  public async predictWithModel(predictDto: PredictDto) {
    ...
    try {
      const response = await clar.post(`v2/models/custom_model/outputs`, JSON.stringify(data))
      return response?.data
    } catch (err) {
      return err
    }
